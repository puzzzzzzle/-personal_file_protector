...
public:
    HANDLE hPip;
    static char pipStaticBuff[40960];
public:
...
public:
    ...
    std::string pip_read_single() {
        pip_read(pipStaticBuff,sizeof (pipStaticBuff));
        return std::string(pipStaticBuff);
    }
    int pip_read() {
        DWORD nLen;
        //从管道读取信息
...
//        DEBUG("[write] write to pip:" << writeStr)
    ...
    int pip_write(const std::string &code) {
        return pip_write(code.c_str(),code.size());
    }
